...
This time, the relatedness matrix was generated using SNP variants called from the samples' WGBS data using `BS-Snper`
```{r}
...
Using a SNP-based relatedness matrix produced very similar (if slightly more conservative) results to the run using a parent-id-based matrix (see [11-diff-methyl-MACAU.Rmd](./11-diff-methyl-MACAU.Rmd)) -- 624 DMLs using SNP-based, 799 DMLs using ID-based. On top of just DML counts, both runs produced nearly identical estimability of heritability. Using an ID-based matrix, roughly 7% (59/799) of DMLs had estimable heritability (`h2` =/ 0,1, or NA), and here we found the same proportion -- 44/624, or ~7% of DMLs had `h2` values =/ 0,1, or NA. These minor differences suggest that collinearity in the relatedness-treatment is either not the source of `h2` problems OR that using a SNP-based approach didn't sufficiently address the collinearity (also plausible, since it's essentially part of the experimental design, with offspring produced from either exclusively Control or exclusively Exposed parents).
